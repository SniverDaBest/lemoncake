use alloc::vec::Vec;
use zune_png::{zune_core::{colorspace::ColorSpace, result::DecodingResult}, PngDecoder};
use crate::FRAMEBUFFER;

pub fn decode(data: &[u8]) -> DecodingResult {
    return PngDecoder::new(data).decode().expect("Unable to decode PNG data!");
}
...
pub fn draw_png(data: &[u8], x: usize, y: usize) {
    ...
    let (width, height) = decoder.get_dimensions().expect("Unable to get image dimensions!");
