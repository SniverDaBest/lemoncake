use crate::{allocator::{alloc_pages_user, alloc_user}, error, syscall::jump_to_usermode};
use core::{fmt, slice};
...
impl Process {
    ...
    pub fn init(&mut self, mapper: &mut impl Mapper<Size4KiB>, frame_allocator: &mut impl FrameAllocator<Size4KiB>) -> Result<(), ProcInitError> {
    ...
    pub fn alloc_stack(&mut self, mapper: &mut impl Mapper<Size4KiB>, frame_allocator: &mut impl FrameAllocator<Size4KiB>, pages: usize) -> Option<VirtAddr> {
        return alloc_pages_user(mapper, frame_allocator, pages);
    }

    pub unsafe fn switch(&mut self, pages: usize, mapper: &mut impl Mapper<Size4KiB>, frame_allocator: &mut impl FrameAllocator<Size4KiB>) {
