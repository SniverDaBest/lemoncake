...
pub fn draw_glyph(
    ...
    for &px in &bitmap {
        ...
        final_glyph.push(BltPixel::new(blended_r, blended_g, blended_b));
    }

    buf.draw_bitmap(final_glyph.as_ref(), metrics.width, metrics.height, x, y).expect("Unable to draw glyph!");
}
